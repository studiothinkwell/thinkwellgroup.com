...
$id = project_category_get($_REQUEST , 'project_category_id' , 0 );
  ...
  $query = " SELECT d.`category_id`, d.`category_name`  FROM druprojectcategory d WHERE d.`category_id` = '$id' ";
...
<form name="drupal_form" action="index.php?q=project_category/save" method="post" enctype="multipart/form-data" onsubmit="return modifyUploadNo()">
    <table>
        <tr>
            <td valign="top">
            ...
            <td>
                <input id="category_name" type="text" style="width:400px" name="category_name" value="<?php echo $row['category_name'] ?>" maxlength="64" />
            </td>
        </tr>
        ...
        <tr>
            <td></td>
            <td><img height="100" width="100" src="misc/project_category_background/<?php echo $row['category_background'] ?>" /></td>
        </tr>
    ...
    <input type="hidden" name="project_category_id" value="<?php echo $row['category_id'] ?>" />
